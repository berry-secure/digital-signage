...
log() {
  printf '[signaldeck-golden] %s\n' "$*"
}

require_root() {
  if [[ "${EUID}" -ne 0 ]]; then
    printf 'Run this script with sudo on the Raspberry Pi golden image source card.\n' >&2
    exit 1
  fi
}
...
install_player() {
  log "Installing Signal Deck player from local repository."
  SIGNALDECK_SOURCE_DIR="${REPO_ROOT}" \
  SIGNALDECK_REF="${SIGNALDECK_REF:-codex/rpi-video-premium-player}" \
  SIGNALDECK_WEBUI_PASSWORD="${WEBUI_PASSWORD}" \
    bash "${SCRIPT_DIR}/install-video-premium.sh"
}
...
reset_clone_specific_state() {
  ...
  install -d -m 0755 "${CONFIG_DIR}" "${STATE_DIR}" "${BOOT_DIR}"
...
print_summary() {
  log "Golden image source is ready."
  ...
  printf '  - unique player serial from Raspberry Pi CPU serial\n'
